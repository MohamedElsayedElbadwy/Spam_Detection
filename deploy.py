...
TEXTS = {
    "EN": {
        "page_title":   "Spam Detector",
        "hero_sub":     "Powered by Multinomial Naive Bayes · Instant email & SMS classification",
        ...
        "step_cls":     "Running Naive Bayes classifier…",
        "step_done":    "Done!",
        "result_spam":  "🚨  SPAM",
        "result_ham":   "✅  NOT SPAM",
        ...
        "prob_ham":     "Legitimate probability",
        ...
        "chart_conf":   "Confidence per Analysis",
    ...
    "AR": {
        ...
        "hero_sub":     "مدعوم بـ Naive Bayes متعدد الحدود · تصنيف فوري للبريد والرسائل",
        ...
        "step_cls":     "تشغيل مصنف Naive Bayes…",
        "step_done":    "اكتمل!",
        ...
        "chart_conf":   "درجة الثقة لكل تحليل",
...
# preview border adapts to text direction
...
.stat-box{{
    ...
    padding:0.62rem 1rem;text-align:center;min-width:92px;
}}
.stat-value{{font-size:1.28rem;font-weight:700;color:{STAT_VAL};}}
.stat-key{{font-size:0.66rem;color:{STAT_KEY};text-transform:uppercase;letter-spacing:0.07em;}}
...
def load_model():
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        df["v2"], df["spam"], train_size=0.8, random_state=42
    )
    vec = CountVectorizer()
    ...
    with open("naive_bayes_model.pkl", "rb") as f:
        mdl = pickle.load(f)
    if hasattr(mdl, "feature_count_") and mdl.feature_count_.shape[1] != len(vec.vocabulary_):
        mdl = MultinomialNB()
        mdl.fit(Xtr, y_train)
    acc = accuracy_score(y_test, mdl.predict(Xte))
    ds  = {"total": len(df), "ham": int((df["spam"]==0).sum()), "spam": int((df["spam"]==1).sum())}
    return mdl, vec, acc, ds

with st.spinner("Loading model…"):
    model, vectorizer, model_acc, ds = load_model()
...
# ── Tabs ──────────────────────────────────────────────────────────────────────
tab_cls, tab_hist, tab_dash = st.tabs([T["tab_classify"], T["tab_history"], T["tab_dashboard"]])
    ...
    with cmain:
        ...
        with ca:
            st.markdown(
                ...
                f'<div class="accuracy-badge">{model_acc*100:.2f}%</div></div>',
                unsafe_allow_html=True)
        with cb:
            st.markdown(
                f'<div class="stat-row">'
                f'<div class="stat-box"><div class="stat-value">{ds["total"]:,}</div>'
                f'<div class="stat-key">{T["total_lbl"]}</div></div>'
                f'<div class="stat-box"><div class="stat-value" style="color:#34d399;">{ds["ham"]:,}</div>'
                f'<div class="stat-key">{T["ham_lbl"]}</div></div>'
                f'<div class="stat-box"><div class="stat-value" style="color:#f87171;">{ds["spam"]:,}</div>'
                f'<div class="stat-key">{T["spam_lbl"]}</div></div>'
                f'</div>',
                unsafe_allow_html=True)
        ...
        user_input  = st.text_area("Message", placeholder=T["placeholder"], height=160, label_visibility="collapsed")
        predict_btn = st.button(T["analyse_btn"], use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)
...
with tab_hist:
    ...
    if not history:
        ...
        rows = "".join(
            ...
            f'<td style="font-size:0.8rem;">'
            f'{e["sp_pct"] if e["verdict"]=="spam" else e["ha_pct"]:.1f}%</td>'
        ...
        st.markdown(
            ...
            f'<th>{T["col_time"]}</th><th>{T["col_result"]}</th>'
            f'<th>{T["col_conf"]}</th><th>{T["col_msg"]}</th>'
    ...
    if not history:
        ...
        with ch_pie:
            ...
            donut = go.Figure(go.Pie(
                ...
                hole=0.58,
                marker=dict(
                    colors=["#ef4444", "#34d399"],
                    line=dict(color=PLT_PAPER, width=2),
                ),
            ...
            donut.update_layout(
                paper_bgcolor=PLT_PAPER, plot_bgcolor=PLT_PAPER,
                font=dict(color=PLT_FONT),
                showlegend=False,
        ...
        with ch_bar:
            st.markdown(f'<div class="section-label">{T["chart_conf"]}</div>', unsafe_allow_html=True)
            df_h = pd.DataFrame([{
                "n":    i + 1,
                "conf": h["sp_pct"] if h["verdict"] == "spam" else h["ha_pct"],
                "type": T["spam_lbl"] if h["verdict"] == "spam" else T["ham_lbl"],
                "ts":   h["timestamp"],
            } for i, h in enumerate(history)])

            bar_fig = px.bar(
                df_h, x="n", y="conf", color="type",
                color_discrete_map={T["spam_lbl"]: "#ef4444", T["ham_lbl"]: "#34d399"},
                labels={"n": "#", "conf": T["col_conf"], "type": T["col_result"]},
                custom_data=["ts", "type"],
            )
            bar_fig.update_traces(
                hovertemplate=(
                    "%{customdata[1]}<br>%{customdata[0]}<br>"
            ...
            bar_fig.update_layout(
                ...
                yaxis=dict(
                    showgrid=True, gridcolor=PLT_GRID,
                    color=PLT_FONT, range=[0, 107], title="",
                ),
                legend=dict(
                    orientation="h", yanchor="bottom", y=1.02,
                    bgcolor="rgba(0,0,0,0)", font=dict(color=PLT_FONT),
                ),
                margin=dict(t=32, b=5, l=5, r=5),
                height=268,
                bargap=0.18,
            )
